...
export type SortKey = "rank" | "change" | "name" | "dtf" | "gap" | "gain" | "near" | "leg" | "run" | "run7" | "wind" | "spd7" | "vdh";
...
const value = (b: Row, key: SortKey): number | string | null =>
  key === "rank" ? b.rank : key === "change" ? b.rank_change : key === "name" ? surname(b.team.name) : key === "dtf" ? b.dtf_nm
  : key === "gap" ? b.gap_nm : key === "gain" ? b.gain24_nm : key === "near" ? b.vs_near_nm : key === "leg" ? b.spd4 : key === "run" ? b.run24_nm : key === "run7" ? b.run7_nm : key === "wind" ? (b.wind_ratio ?? null) : key === "spd7" ? b.spd7 : b.vs_vdh_days;
// Bigger is better for gains, runs, speed and the ghost gap, so those open longest or fastest first; the rest open ascending.
export const defaultDir = (key: SortKey): SortDir => (key === "change" || key === "gain" || key === "near" || key === "leg" || key === "run" || key === "run7" || key === "wind" || key === "spd7" || key === "vdh" ? "desc" : "asc");
export function sortBoats<T extends Row>(boats: T[], key: SortKey, dir: SortDir): T[] {
  const sign = dir === "asc" ? 1 : -1;
  return [...boats].sort((a, c) => {
    const x = value(a, key), y = value(c, key);
