...
const value = (b: Row, key: SortKey, win: Win): number | string | null =>
  ...
  : key === "gap" ? b.gap_nm : key === "gain" ? b.gain24_nm : key === "near" ? b.vs_near_nm : key === "run" ? (win === "4h" ? b.spd4 : win === "7d" ? b.run7_nm : b.run24_nm) : key === "spd7" ? b.spd7 : b.vs_vdh_days;
// Bigger is better for gains, runs, speed and the ghost gap, so those open longest or fastest first; the rest open ascending.
export const defaultDir = (key: SortKey): SortDir => (key === "change" || key === "gain" || key === "near" || key === "run" || key === "spd7" || key === "vdh" ? "desc" : "asc");
