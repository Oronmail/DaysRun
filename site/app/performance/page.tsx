  ...
  return <Shell active="Performance" dateline={dateline(fleet.as_of, fleet.race_day)} title="PERFORMANCE" note={top ? `${top.b.team.first_name} gets the most from the wind` : undefined}>
    ...
    <div className="stack" style={{ display: "grid", gridTemplateColumns: "800px minmax(0,1fr)", gap: 40 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}><div className="rule-title"><div className="label">Speed for the wind</div><div className="small" style={{ fontStyle: "italic" }}>boat speed ÷ model wind speed, legs sailed in 8–25 kt, since the start</div></div>
        <div className="panel"><svg width={W} height={rated.length * 24 + 26} viewBox={`0 0 ${W} ${rated.length * 24 + 26}`} style={{ display: "block" }} role="img" aria-label="Boat speed as a share of wind speed, by skipper">
