...
import { ClassTag, Tri } from "@/components/Who";
...
export default async function Page() {
  ...
  return <Shell active="Skippers" dateline={<Dateline asOf={fleet.as_of} raceDay={fleet.race_day} />} title="SKIPPERS" note="one page per boat: track, speed, runs, ghosts">
    ...
    <div className="cards">{boats.map(b => <Link key={b.team_id} href={`/skipper/${b.team_id}`} className="card">
      ...
      <div className="mont" style={{ fontSize: 16, fontWeight: 600, lineHeight: 1.2 }}>{b.team.name} <span style={{ fontWeight: 500, color: "var(--graphite)", fontSize: 11 }}>{b.team.country_code}</span><ClassTag team={b.team} /></div>
