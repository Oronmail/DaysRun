...
import { racePlaceTip, raceGapTip, type Tip } from "@/lib/tips";
...
function Chart({ title, note, series, names, leaderId, ymax, yticks, ylab, invert, tip, width = 640, height = 400 }: { tip: (first: string, v: number, iso: string) => Tip; title: string; note: string; series: Map<number, Pt[]>; names: Map<number, string>; leaderId: number; ymax: number; yticks: number[]; ylab: (v: number) => string; invert?: boolean; width?: number; height?: number }) {
  ...
  const S = 14, targets: Target[] = [...series.entries()].sort((a, b) => (a[0] === leaderId ? 1 : 0) - (b[0] === leaderId ? 1 : 0)).flatMap(([id, p]) =>
    p.map(q => ({ x: X(q.t) - S / 2, y: Y(q.v) - S / 2, w: S, h: S, point: true, dot: { x: X(q.t), y: Y(q.v) }, tip: tip(names.get(id) ?? "", q.v, new Date(q.t).toISOString()) })));
  return <div style={{ display: "flex", flexDirection: "column", gap: 10 }}><div className="rule-title"><div className="label">{title}</div><div className="small" style={{ fontStyle: "italic" }}>{note}</div></div>
    <div className="panel"><Tips width={width} height={height} targets={targets}><svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: "block" }} role="img" aria-label={title}>
...
export default function RaceChart({ days, boats }: { days: DailyPlace[]; boats: BoatStat[] }) {
  ...
  return <div className="stack" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 40 }}>
    <Chart title="Places, day by day" note="at 00:00 UTC each day · point at a line, or at a day on it" tip={racePlaceTip} series={place} names={names} leaderId={leaderId} ymax={n} invert yticks={[1, 4, 8, 12, n]} ylab={v => String(v)} />
    <Chart title="Gap to the leader" note="nautical miles behind, by distance to finish" tip={raceGapTip} series={gap} names={names} leaderId={leaderId} ymax={gmax} yticks={[0, gmax / 4, gmax / 2, 3 * gmax / 4, gmax]} ylab={v => (v === 0 ? "0" : `−${Math.round(v)}`)} />
  </div>;
}
