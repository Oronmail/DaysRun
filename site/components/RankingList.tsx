...
export default function RankingList({ boats }: { boats: RankRow[] }) {
  const run = (b: BoatStat) => `${b.run24_bridged && b.run24_nm != null ? "\u2265\u2009" : ""}${nm(b.run24_nm)} nm / 24 h · ${nm(b.run7_nm)} nm / 7 d`;
  ...
  return <div>
    <div className="list-sort mont"><label htmlFor="list-sort">Sort by</label>
      <select id="list-sort" value={sort.key} onChange={e => { const key = e.target.value as SortKey; setSort({ key, dir: defaultDir(key) }); }}>{phoneSortOptions().map(o => <option key={o.key} value={o.key}>{o.label}</option>)}</select>
    ...
    <div style={{ minWidth: 0 }}>
      ...
      <div className="small" style={{ fontSize: 12, marginTop: 4, whiteSpace: "nowrap" }}>{b.spd4 != null && !b.stale && <>latest leg {kn(b.spd4)} kt<Course deg={b.cmg4 ?? 0} /> · </>}{b.wind_ratio != null ? `${Math.round(b.wind_ratio * 100)}% for the wind` : "not rated for the wind yet"}</div>
    </div>
    <div className="num" style={{ textAlign: "right", whiteSpace: "nowrap" }}>
      ...
      <div className="small" style={{ fontSize: 11, fontWeight: b.fleet_best24 ? 600 : 400 }}>{run(b)}</div>
