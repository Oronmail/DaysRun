...
export default function SpeedLog({ log, endAt, width = 728, height = 176, max = 8 }: { log: (number | null)[]; endAt: string; width?: number; height?: number; max?: number }) {
  ...
  const gold = highest(log, v => v.toFixed(1));                                           // the fastest leg of the seven days (every bar that prints the top figure), not the latest
  const targets = log.map((v, i) => ({ x: L + i * slot, y: T, w: slot, h: ph, tip: legTip(v, new Date(legEnd(i)).toISOString(), gold.has(i)) }));
  return <Tips width={width} height={height} targets={targets}><svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: "block" }} role="img" aria-label="Average speed on each 4-hour leg over the last 7 days, in knots">
    ...
    {log.map((v, i) => { const x = L + i * slot + 1.25, startsDay = new Date(legEnd(i) - SLOT_MS).getUTCHours() === 0, top = gold.has(i); return <g key={i}>
      ...
      {v == null
        ...
        : <><rect x={x} y={Y(v)} width={bw} height={Math.max(1.5, T + ph - Y(v))} rx={1.5} fill={top ? "var(--gold)" : "var(--bar)"}></rect>
            <text x={x + bw / 2} y={Y(v) - 3} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={8} fontWeight={top ? 700 : 400} fill="var(--ink)">{v.toFixed(1)}</text></>}
    </g>; })}
  </svg></Tips>;
}
