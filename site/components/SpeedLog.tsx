  ...
  return <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: "block" }} role="img" aria-label="Average speed on each 4-hour leg over the last 7 days, in knots">
    ...
    {log.map((v, i) => { const x = L + i * slot + 1.25, startsDay = new Date(legEnd(i) - SLOT_MS).getUTCHours() === 0, top = gold.has(i); return <g key={i}>
      ...
      {v == null
        ...
        : <><rect x={x} y={Y(v)} width={bw} height={Math.max(1.5, T + ph - Y(v))} rx={1.5} fill={top ? "var(--gold)" : "var(--bar)"}><title>{`${v.toFixed(1)} kt · leg ending ${dayMonTime(new Date(legEnd(i)).toISOString())} UTC`}</title></rect>
