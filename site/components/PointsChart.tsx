...
import { project, viewHeight, landPaths, latLabel, lonLabel, type View } from "@/lib/geo";
...
export default function PointsChart({ view, points, labels = [] }: { view: View; points: Point[]; labels?: { name: string; lat: number; lon: number; side?: "l" | "r" }[] }) {
  ...
  return <Tips width={W} height={H} targets={targets}>
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
      ...
      {[...Array.from(new Set(points.map(p => Math.floor(p.lon / 5) * 5)))].map(lo => { const [x] = project(view.lat1, lo, view); return <text key={lo} x={x + 3} y={12} fontFamily="var(--font-mono)" fontSize={9} fill="var(--graphite)" className="map-halo">{lonLabel(lo)}</text>; })}
