...
// Everything is gated except Next's static assets and the site's icons (no race data in them), Vercel's own analytics paths, the
// health address an outside monitor watches (two times and a verdict), and the worker's revalidation ping and Vercel's cron call,
// each of which carries its own secret.
export const config = { matcher: "/((?!_next/static|_next/image|_vercel|favicon.ico|icon.svg|apple-icon.png|api/revalidate|api/cron|api/health).*)" };
