...
- **If GitHub's schedule does not fire:** GitHub's scheduled workflows are best-effort — late under load, and on a new or quiet repository they can fail to start at all. Vercel Cron calls `/api/cron/worker` every hour at :20 (`site/vercel.json`); it starts the workflow through GitHub's API only if no run began in the last 45 minutes, so it is silent while GitHub's own schedule works. It needs two Vercel env vars: `CRON_SECRET` and `GH_DISPATCH_TOKEN` (a fine-grained token for this repository only, Actions read and write; renew it before it expires). Check it: Vercel → the project → Cron Jobs → logs.
